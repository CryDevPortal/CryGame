...
private:
  ...
  void _M_allocate_buffer() {
    ...
    while (_M_len > 0) {
			...
			// ensure alignment on PS3-ppu when using for example stable sort
      _M_buffer = (_Tp*) _STLP_CLASS_DECLSPEC __new_alloc::allocate(_M_len * sizeof(_Tp), __alignof__(_Tp) );
#else
