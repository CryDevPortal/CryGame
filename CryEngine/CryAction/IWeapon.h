...
{
	virtual ~IFireMode(){}
	virtual void Init(IWeapon *pWeapon, const struct IItemParamsNode *params, uint32 id) = 0;
	...
	virtual void ModifyParams(bool modify, bool modified = true) = 0;
	...
	virtual const char* GetChangeFiremodeAction(const char* newFiremode, bool zoomed) { return NULL; };
  ...
  virtual float GetFireRate() const = 0;
	...
	virtual bool GetCrosshairPosDir(Vec3 &pos, Vec3& dir, float& distToTarget) const {CRY_ASSERT(false); return false;}
  ...
  virtual int GetCurrentBarrel() const = 0;
	...
	virtual void PatchSpreadMod(const SSpreadModParams &sSMP) = 0;
	virtual void ResetSpreadMod() = 0;

	virtual void PatchRecoilMod(const SRecoilModParams &sRMP) = 0;
...
struct IWeaponSharedData
{
	virtual ~IWeaponSharedData(){}
...
struct IWeaponEventListener
{
	...
	virtual void OnZoomChanged(IWeapon* pWeapon, bool zoomed, int idx) = 0;
};
...
struct IWeapon
{
  ...
  virtual void SetDestinationEntity(EntityId targetId) = 0;
	...
	// Summary
	//   Retrieves a specified zoom mode name, by idx
	// Parameters
	//   idx - index of the specified zoom mode
	// See Also
	//   GetNumOfZoomModes, GetZoomModeIdx
	virtual const char *GetZoomModeName(int idx) const = 0;
	...
	virtual bool PredictProjectileHit(IPhysicalEntity *pShooter, const Vec3 &pos, const Vec3 &dir,
		...
		Vec3* pTrajectory = 0, unsigned int* trajectorySizeInOut = 0, float timeStep = 0.24f) const = 0;
...
*/	
	...
	virtual void RequestLock(EntityId id, int partId = 0) = 0;
	...
	virtual void RequestStartMeleeAttack(bool weaponMelee, bool boostedAttack) = 0;
