...
enum CA_AssetFlags
{
	...
	// Description:
	//		If this is true, then the LMG has been processed at loading time.
	CA_ASSET_PMG_VALID	= 0x080,

	// 0x100 and 0x200 are used for development and testing

	// Description:
	//		If this is true, then the asset has been processed at loading time.
	CA_ASSET_PROCESSED	= 0x400,
	// Description:
...
enum CA_AnimationFlags
{
	...
	CA_ALLOW_ANIM_RESTART			=0x000080, 
	...
	CA_KEYFRAME_SAMPLE_30Hz	    =0x000100, //don't interpolate between 2 keyframes (do only 30hz sampling)
	
	//Don't allow multilayer animations when this flag is set in Layer0
	CA_DISABLE_MULTILAYER       =0x000200,  
	...
	CA_REMOVE_FROM_FIFO	= 0x004000,  

	CA_FULL_ROOT_PRIORITY	= 0x008000,  
	...
	CA_FORCE_TRANSITION_TO_ANIM	= 0x010000,  
};
...
struct SAnimMemoryTracker
{
	...
	uint32 m_nTotalCharMememory;
	uint32 m_numTSkinInstances;
	uint32 m_nTotalSkinMememory;  
	uint32 m_numModels;
	uint32 m_nTotalMMememory;
	...
	SAnimMemoryTracker()
	{
		...
		m_nTotalCharMememory=0;
		m_numTSkinInstances=0;
		m_nTotalSkinMememory=0;  
		m_numModels=0;
		m_nTotalMMememory=0;
	}
...
struct CryCharAnimationParams
{
	CryCharAnimationParams( int _nLayerID=0, uint32 _nFlags=0) : m_fTransTime(-1.0f),
		m_fKeyTime(-1.0f), m_fPlaybackSpeed(1.0f),
		m_fAllowMultilayerAnim(1.0f), m_nLayerID( _nLayerID),
	...
	int32 m_nLayerID;
	// every layer can override the previous layer with certain transition weight
	f32 m_fLayerWeight;
...
ILINE bool operator==(const MotionParam& lhs, const MotionParam& rhs)
{
	// doesn't check desc or blendspace, since they're not serialized
...
struct SParametric
{
	int16		m_nParametricID;												//if >=0 then this is a "locomotion group"
	int16		m_numAnims;											//number of used animations per locomotion group

	int16		m_nAnimID[MAX_LMG_ANIMS];
	int16		m_nSegmentCounter[MAX_LMG_ANIMS];
	...
	void Serialize(TSerialize ser)
	{
		if (ser.GetSerializationTarget() != eST_Network)
		{
			ser.BeginGroup("SParametric");

			ser.BeginGroup("Params");
			for (int32 i=0; i<eMotionParamID_COUNT; i++)
			{
				string serializeName;
				serializeName.Format("motionParams%i", i);
				ser.ValueWithDefault(serializeName.c_str(), m_params[i], MotionParam());
			}
			ser.EndGroup();

			ser.Value("nParametricID", m_nParametricID);
...
struct CAnimation
{
	SParametric *m_Parametric;
	int16 m_nAnimDirID0;								
	int16 m_nAnimDirID1;				

	int16		m_nAnimID;
	int16		m_nSegmentCounter;
	...
	void Serialize(TSerialize ser)
	{
		if (ser.GetSerializationTarget() != eST_Network)
		{
			...
			ser.Value("nAnimID", m_nAnimID);

			ser.ValueWithDefault("nAnimDirID0", m_nAnimDirID0, (int16)-4);
			ser.ValueWithDefault("nAnimDirID1", m_nAnimDirID1, (int16)-4);
			...
			ser.ValueWithDefault("nSegHighest", m_nSegHighest, (uint16)1);
			ser.EndGroup();

			//if (m_Parametric)
			//{
			//	ser.BeginGroup("SParametric");
			//	m_Parametric->Serialize(ser);
			//	ser.EndGroup();
			//}
