...
struct FileSerializationHelper
{
	...
	{
		CRY_ASSERT( !"Binary serialization of size_t data types is not supported!" );
	}
};
...
{
	SAnimationMovement() : translation(0.0f, 0.0f, 0.0f), rotation(1.0f, 0.0f, 0.0f, 0.0f), duration(0.0f) {}
	SAnimationMovement( const Vec3& t, const Quat& r, float d) : translation(t), rotation(r), duration(d) {}
	Vec3 translation;
	Quat rotation;
	float duration;
...
struct IAnimationStateNode
{
	...
	// Description:
	//		Called when the AnimationGraph is paused/resumed. The fOverrideTransTime parameter can be used for overriding
	//		the default transition time on nodes that use transition times (like AGAnimation, AGAdditive, etc.) just in case 
	//		the system that pauses the AG needs it) the default value -1.0f nullifies its effect
	...
	virtual void Update( SAnimationStateData& data ) {CRY_ASSERT(false);}
	// Description:
